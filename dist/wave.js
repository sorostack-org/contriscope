...
exports.suggestWaveComplexity = suggestWaveComplexity;
function suggestWaveComplexity(input) {
    const { config } = input;
    ...
    const fileMatches = [...haystack.matchAll(/(\d+)\s*\+?\s*files?\b/g)].map((match) => Number.parseInt(match[1], 10));
...
function buildReasoning(level, score, highHits, mediumHits, trivialHits) {
    const parts = [];
